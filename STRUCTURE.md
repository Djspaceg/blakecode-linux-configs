...
├── bashrc                           → ~/.bashrc
├── scripts/
│   ├── shell_aliases.sh             (shared, sourced by rc files)
│   ├── prepare-rc-files.zsh         (sourced by zshenv)
│   └── *.sh                         (utility scripts)
├── screenrc                         → ~/.screenrc
├── functions/
│   ├── custom_hostname
...
4. **Shared aliases** → `scripts/shell_aliases.sh`
